...
class DataProcess:
	...
	def compute_phi(self, cutoff):
		...
		phi_params = phi_ctrl_pts(y = self.sorted_y_train)
		y_phi = phi(y = self.sorted_y_train, ctrl_pts = phi_params) # relevance function finished
		y_phi = np.asarray(y_phi)
		self.phi_zip = zip(y_phi, self.sorted_y_train)

		# computation of control points
		all_indexes = [*range(len(self.sorted_y_train))]
